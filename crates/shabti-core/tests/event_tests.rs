use shabti_core::event::{
    AdaptiveThreshold, Boundary, consecutive_distances, cosine_distance, cosine_similarity,
    detect_boundaries, detect_boundaries_adaptive, detect_time_gaps, merge_boundaries,
    segment_events,
};
...
fn cosine_distance_identical_is_zero() {
    ...
    assert!(
        d.abs() < 1e-6,
        "distance of identical vectors should be 0, got {d}"
    );
}
...
fn cosine_distance_orthogonal_is_one() {
    ...
    assert!(
        (d - 1.0).abs() < 1e-6,
        "distance of orthogonal vectors should be 1, got {d}"
    );
}
...
fn consecutive_distances_length() {
    ...
    assert_eq!(
        distances.len(),
        2,
        "N embeddings should produce N-1 distances"
    );
}
...
fn consecutive_distances_detects_topic_shift() {
    ...
    assert!(
        ...
        "topic shift distance should be larger than within-topic"
    );
    assert!(
...
fn adaptive_threshold_homogeneous_data_no_boundaries() {
    ...
    assert!(
        boundaries.is_empty(),
        "homogeneous data should have no boundaries, got {:?}",
        boundaries
    );
}
...
fn adaptive_threshold_clear_peak() {
    ...
    assert_eq!(
        boundaries.len(),
        1,
        "should detect one boundary at the peak"
    );
...
fn adaptive_threshold_window_size_effect() {
    ...
    let small_window = AdaptiveThreshold {
        window_size: 2,
        sensitivity: 2.0,
    };
    let large_window = AdaptiveThreshold {
        window_size: 4,
        sensitivity: 2.0,
    };
    ...
    assert!(
        b1.iter().any(|b| b.index == 2),
        "small window should detect peak"
    );
    assert!(
        b2.iter().any(|b| b.index == 2),
        "large window should detect peak"
    );
}
...
fn adaptive_threshold_fallback_for_small_data() {
    ...
    assert!(
        boundaries.iter().any(|b| b.index == 0),
        "should detect high distance with fallback"
    );
}
...
fn evaluate_boundaries_missed_detection() {
    ...
    assert!(
        (precision - 1.0).abs() < 1e-6,
        "detected boundary is correct"
    );
    assert!(
        (recall - 0.5).abs() < 1e-6,
        "only 1 of 2 ground truth found"
    );
}
